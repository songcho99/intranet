...
var pageNum = "<%=pageNum%>";
$(function(){
   list(mreader,null,null,pageNum);
   
    $(document).on("click","#sendMessage",function(){
         window.open("message/message_writeform.jsp","","width=600px,height=500px,left=600px,top=100px");
     ...
     $("#srchBtn").click(function(){
         ...
         list(mreader,category,searchtxt,pageNum);
      });
...
function list(mreader,category,searchtxt,pageNum){
         var str = "";
         var write="";
   $.ajax({
      type:"post",
      url:"message/message_tolist_action.jsp",
      dataType:"json",
      data:{"mreader":mreader,"category":category,"searchtxt":searchtxt,"pageNum":pageNum},
      success:function(data){
            ...
            str+="<table id='totablelist'>";
            str+="<tr>";
            str+="<th style='width:10%;'>보낸사람</th>";
            ...
            $.each(data, function(i,item){
               ...
               startPage = item.startPage;
                  ...
                  if(item.totalcount==0){
                    str+="<tr><td><b>받은 쪽지가 없습니다</b></td></tr>";
                  
                  }else{
                     if(item.mreaderdel==0){
                     
             str+="<tr>";            
            str+="<td style='text-align:center;'>"+item.writer+"("+item.writerid+")</td>";
            str+="<td><a href='<%=root%>/startmain.jsp?content=message/message_select.jsp?mnum="+item.mnum+"&pageNum="+item.currentPage+"&message=2' style='color:black'>"+item.mcontent+"</a></td>";
